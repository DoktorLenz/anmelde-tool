...
describe('On change of userdata', () => {
  ...
  const eventsSubject: Subject<OidcClientNotification<OidcUserData>> =
    new Subject();
  ...
  beforeEach(() =>
    MockBuilder(UserDataService, AppModule)
      ...
      .mock(PublicEventsService, {
        registerForEvents: jasmine
          .createSpy()
          .and.returnValue(eventsSubject.asObservable()),
      })
  ...
  it('should save user data in local storage', () => {
    ...
    const userDataSpy = ngMocks.stubMember(
      localStorageService,
      'userData',
      jasmine.createSpy(),
      'set'
    ...
    expect(userDataSpy).toHaveBeenCalledWith({
      ...oidcUserData,
...
describe('When hasRole is called', () => {
  ...
  it('should return observable with value true when user has role', fakeAsync(() => {
    ...
    ngMocks
      .stubMember(localStorageService, 'userData', jasmine.createSpy(), 'get')
